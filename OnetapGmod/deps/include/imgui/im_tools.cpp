...
bool ImGui::Hotkey(const char* label, uint32_t* k, const ImVec2& size_arg, uint32_t none_key, const char* none_str)
{
	...
	ImVec2 size = ImGui::CalcItemSize(size_arg, ImGui::CalcItemWidth()/1.2, label_size.y + style.FramePadding.y * 2.0f);
	...
	ImVec2 size1 = ImGui::CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);

	const ImRect bb(pos, pos + size);
	ImGui::ItemSize(size1, style.FramePadding.y);
	if (!ImGui::ItemAdd(bb, id))
		return false;
	char buf_display[64] = "None";
	strcpy_s(buf_display, none_str);
	window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 0), ImVec2(frame_bb.Max.x + 0, frame_bb.Max.y + 0), ImColor(29, 28, 32, 244), 4, 15);
	window->DrawList->AddRect(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 0), ImVec2(frame_bb.Max.x + 0, frame_bb.Max.y + 0), ImColor(56, 54, 59, 255), 4, 15, 1.000000);
	window->DrawList->AddRect(ImVec2(frame_bb.Min.x + -1, frame_bb.Min.y + -1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), ImColor(17, 17, 22, 255), 4, 15, 1.000000);
	window->DrawList->AddRect(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + 1), ImVec2(frame_bb.Max.x + -1, frame_bb.Max.y + -1), ImColor(17, 17, 22, 255), 4, 15, 1.000000);

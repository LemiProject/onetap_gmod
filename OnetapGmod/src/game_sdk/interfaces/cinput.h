...
struct camera_third_data_t
{
	...
	float   m_flDist;
	float   m_flLag;
	c_vector  m_vecHullMin;
	c_vector  m_vecHullMax;
};
...
public:
	...
	q_angle		m_angPreviousViewAngles;
	...
	camera_third_data_t* m_pCameraThirdData;
	...
	virtual		void* GetUserCmd(int sequence_number);

	virtual		void		MakeWeaponSelection(void* weapon);

	virtual		float		KeyState(int32_t* key);
	virtual		int			KeyEvent(int down, int32_t keynum, const char* pszCurrentBinding);
